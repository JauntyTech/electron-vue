...
When running `yarn` a post install script is also executed, which will `yarn` inside the `app` directory as well.
...
If you run into errors during `yarn` about `node-gyp`, then you most likely do not have the proper build tools installed on your system. Build tools include items like Python and Visual Studio. Thanks to [@felixrieseberg](https://github.com/felixrieseberg), there are a few packages to help simplify this process.
